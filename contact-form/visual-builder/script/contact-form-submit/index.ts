...
function getSafeRedirectUrl(url: string): string {
  ...
  const normalizedRedirectUrl = url.trim();
  ...
  try {
    ...
    anchor.href = normalizedRedirectUrl;
...
export const contactFormSubmit = (selector: string, event: JQuery.Event) => {
  ...
  const $thisInputs = $thisContactForm.find<HTMLInputElement>(
  ...
  // Use form_unique_id (module UUID) instead of form_unique_num (orderIndex) to ensure
  // globally unique field names across all Theme Builder areas (Header, Body, Footer).
  // Use .attr() instead of .data() to get the raw attribute value, and fall back to
  // extracting UUID from container ID if needed.
  let formUniqueId = $thisContactFormContainer.attr('data-form_unique_id') ?? '';
  if ('' === formUniqueId || null === formUniqueId) {
    // Fallback: extract UUID from container ID (format: et_pb_contact_form_{uuid}).
    const containerId = $thisContactFormContainer.attr('id') ?? '';
    formUniqueId = containerId.replace(/^et_pb_contact_form_/, '') || '';
  }
  let isValid = true;
  let etMessage = '';
  let etFieldsMessage = '';
  ...
  if (recaptchaApi && $thisContactFormContainer.hasClass('et_pb_recaptcha_enabled')) {
    // Sanitize formUniqueId (UUID) by replacing hyphens with underscores to comply with
    // Google reCAPTCHA v3 action name requirements (A-Za-z0-9/_ only).
    const sanitizedFormUniqueId = formUniqueId.replace(/-/g, '_');
    // eslint-disable-next-line @typescript-eslint/no-floating-promises
    recaptchaApi.interaction(`Divi/Module/ContactForm/${sanitizedFormUniqueId}`).then((token: string) => {
      // eslint-disable-next-line @typescript-eslint/no-floating-promises
  ...
  jQuery.when(tokenDeferred).done((token: string) => {
    ...
    $thisInputs.each((i, formFieldEle) => {
      ...
      if (
        !$thisEl.is(':visible') &&
        $thisEl.parents('[data-conditional-logic]').length &&
        'hidden' !== $thisEl.attr('type') &&
        'radio' !== $thisEl.attr('type')
      ) {
      ...
      if (
        ('hidden' === $thisEl.attr('type') || 'radio' === $thisEl.attr('type')) &&
        !$thisEl.parents('.et_pb_contact_field').is(':visible')
      ) {
        hiddenFields.push(originalId);
        return;
      }
      ...
      if (
        ...
        ('' === thisVal || true === unchecked) &&
        !$thisEl.is('[id^="et_pb_contact_et_number_"]')
      ) {
        if (!$thisWrapper) {
    ...
    if (isValid) {
      ...
      $thisContactFormContainer
        .removeClass('et_animated')
        .removeAttr('style')
        .fadeTo('fast', 0.2, () => {
          $thisContactFormContainer.load(
            `${$href} #${$thisContactFormContainer.attr('id')}> *`,
            formData,
