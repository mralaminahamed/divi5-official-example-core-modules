 ...
 */
export const SettingsDesign = ({ groupConfiguration }: Module.Settings.Panel.Props<SignupAttrs>): ReactElement => {
  const useFocusBordersCallback = (params: Module.Settings.Group.VisibleCallback.Params<SignupAttrs>) => {
  ...
  const singleNameFieldCallback = (params: Module.Settings.Field.CallbackParams<SignupAttrs>) => {
    const singleNameField =
      getAttrValue({
  ...
  if (groupConfiguration?.designLayout?.component?.props) {
    set(
      groupConfiguration,
      ['designLayout', 'component', 'props', 'fields', 'fieldAdvancedNamefullwidth', 'visible'],
      singleNameFieldCallback,
    );
  }
