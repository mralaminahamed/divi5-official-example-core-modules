...
import { type Module, type SignupAttrs } from '@divi/types';
 ...
 */
export const SettingsContent = ({ groupConfiguration }: Module.Settings.Panel.Props<SignupAttrs>): ReactElement => {
  const { moduleId } = useContext(moduleContext);

  const { singleNameFieldAttr, enableCustomFieldsAttr, emailServiceAttr } = useSelect(selectStore => ({
    singleNameFieldAttr: selectStore('divi/edit-post').getModuleAttr<SignupAttrs['field']['advanced']['nameField']>(
      moduleId,
      'field.advanced.nameField',
    ),
    enableCustomFieldsAttr: selectStore('divi/edit-post').getModuleAttr<
      SignupAttrs['customFields']['advanced']['enable']
    >(moduleId, 'customFields.advanced.enable'),
  ...
  const emailServiceAccount = useSelect(selectData =>
    selectData('divi/email-marketing').getAccount(selectedProvider, selectedAccountName),
  );
  const accountCustomFields = useSelect(selectData =>
  ...
  const customFieldsNoticeVisible =
    useCustomFieldsVisible && 'on' === enableCustomFields && isCustomFieldsPredefined && isEmpty(accountCustomFields);

  const successActionCallback = (params: Module.Settings.Field.CallbackParams<SignupAttrs>) => {
    const successAction =
      getAttrValue({
  ...
  const notSuccessActionCallback = (params: Module.Settings.Field.CallbackParams<SignupAttrs>) => {
    const successAction =
      getAttrValue({
        attr: params?.attrs?.success?.advanced?.action,
        baseBreakpoint: params?.baseBreakpoint,
  ...
  if (groupConfiguration?.contentFields?.component?.props) {
    set(
      groupConfiguration,
      ['contentFields', 'component', 'props', 'fields', 'customFieldsAdvancedFields', 'render'],
      customFieldsList,
    ); // Usably, We use `visible` prop to hide the field, but in this case we need to use `render`.
    set(
      groupConfiguration,
      ['contentFields', 'component', 'props', 'fields', 'fieldAdvancedNamefieldonly', 'visible'],
      nameFields?.name,
    );
    set(
      groupConfiguration,
      ['contentFields', 'component', 'props', 'fields', 'fieldAdvancedNamefield', 'visible'],
      nameFields?.useSingleNameField,
    );
    set(
      groupConfiguration,
      ['contentFields', 'component', 'props', 'fields', 'fieldAdvancedFirstnamefield', 'visible'],
      nameFields?.showFirstNameField && 'off' === singleNameField,
    );
    set(
      groupConfiguration,
      ['contentFields', 'component', 'props', 'fields', 'fieldAdvancedLastnamefield', 'visible'],
      nameFields?.showLastNameField && 'off' === singleNameField,
    );

    set(
      groupConfiguration,
      ['contentFields', 'component', 'props', 'fields', 'customFieldsAdvancedEnable', 'visible'],
      useCustomFieldsVisible,
    ); // Use Custom Fields.
    set(
      groupConfiguration,
      ['contentFields', 'component', 'props', 'fields', 'customFieldsAdvancedNotice', 'visible'],
      customFieldsNoticeVisible,
  ...
  if (groupConfiguration?.contentSuccessAction?.component?.props) {
    set(
      groupConfiguration,
      ['contentSuccessAction', 'component', 'props', 'fields', 'successAdvancedMessage', 'visible'],
      successActionCallback,
    );
    set(
      groupConfiguration,
      ['contentSuccessAction', 'component', 'props', 'fields', 'successAdvancedRedirectquery', 'visible'],
      notSuccessActionCallback,
    );
    set(
      groupConfiguration,
      ['contentSuccessAction', 'component', 'props', 'fields', 'successAdvancedRedirecturl', 'visible'],
      notSuccessActionCallback,
    );
  }
