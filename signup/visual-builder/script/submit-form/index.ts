...
export const signupSubmitForm = ($submit: JQuery, event: JQuery.Event) => {
  ...
  const emailRegex =
    // eslint-disable-next-line no-useless-escape -- Forward slash in character class doesn't need escape, but keeping for consistency with HTML5 spec.
  ...
  const account = $newsletterContainer
    .find<HTMLInputElement>('input[name="et_pb_signup_account_name"]')
    .val() as string;
  const ipAddress = $newsletterContainer
    .find<HTMLInputElement>('input[name="et_pb_signup_ip_address"]')
    .val() as string;
  ...
  $customFields.each((i, customFieldEle) => {
    ...
    if ('mailchimp' === provider && ['select', 'radio'].indexOf(fieldType) > -1) {
      const $selectedOption =
        'select' === fieldType ? $thisEl.find(':selected') : $thisWrapper.find('input[type="radio"]:checked');
  ...
  if (recaptchaApi && $newsletterContainer.hasClass('et_pb_recaptcha_enabled')) {
    ...
    recaptchaApi.interaction(`Divi/Module/EmailOptin/List/${listId}`).then((token: string) => {
      // eslint-disable-next-line @typescript-eslint/no-floating-promises
