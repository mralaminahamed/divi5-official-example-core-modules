...
import { populatePredefinedFieldsOptions } from './utils';
...
export const SettingsContent = ({
  ...
  const { fieldTitleAttr, emailServiceAttr } = useSelect(selectStore => {
    const parentModuleId = selectStore('divi/edit-post').getParentModuleId(moduleId);

    return {
      fieldTitleAttr: selectStore('divi/edit-post').getModuleAttr<SignupCustomFieldAttrs['fieldItem']['innerContent']>(
        moduleId,
        'fieldItem.innerContent',
      ),
      emailServiceAttr: selectStore('divi/edit-post').getModuleAttr<SignupAttrs['module']['advanced']['emailService']>(
        parentModuleId,
  ...
  const predefinedFields = useSelect(selectData =>
    selectData('divi/email-marketing').getCustomFields(selectedProvider, selectedAccountName, selectedAccountList),
  );
  ...
  const onChangePredefinedSelect = ({ inputValue }: FieldLibrary.Select.OnChangeCallbackParams) => {
    if ('none' !== inputValue) {
      ...
      const attrAdvancedCheckboxOptions = select('divi/edit-post').getModuleAttr(
        moduleId,
        'fieldItem.advanced.checkboxOptions',
      );
      const attrAdvancedRadioOptions = select('divi/edit-post').getModuleAttr(
        moduleId,
        'fieldItem.advanced.radioOptions',
      );
      const attrAdvancedSelectOptions = select('divi/edit-post').getModuleAttr(
        moduleId,
      ...
      dispatch('divi/edit-post').editModuleAttribute({
        ...
        subName: false, // Skip detection - replacing entire attribute value, no subName concept
      });
      ...
      switch (inputType) {
        case 'checkbox': {
          dispatch('divi/edit-post').editModuleAttribute({
            ...
            value: attrAdvancedCheckboxOptions.setIn(
              ['desktop', 'value'],
              populatePredefinedFieldsOptions(predefinedOptionsRaw),
            ),
            subName: false, // Skip detection - replacing entire attribute value, no subName concept
          });
          break;
        ...
        case 'radio': {
          dispatch('divi/edit-post').editModuleAttribute({
            ...
            value: attrAdvancedRadioOptions.setIn(
              ['desktop', 'value'],
              populatePredefinedFieldsOptions(predefinedOptionsRaw),
            ),
            subName: false, // Skip detection - replacing entire attribute value, no subName concept
        ...
        case 'select': {
          dispatch('divi/edit-post').editModuleAttribute({
            ...
            value: attrAdvancedSelectOptions.setIn(
              ['desktop', 'value'],
              populatePredefinedFieldsOptions(predefinedOptionsRaw),
            ),
            subName: false, // Skip detection - replacing entire attribute value, no subName concept
  ...
  const conditionalLogicEnableCallback = (params: Module.Settings.Field.CallbackParams<SignupCustomFieldAttrs>) => {
    const conditionalLogicEnable =
      getAttrValue({
  ...
  const fieldTypeInputCallback = (params: Module.Settings.Field.CallbackParams<SignupCustomFieldAttrs>) => {
    const fieldType =
      getAttrValue({
  ...
  const fieldTypeCheckboxCallback = (params: Module.Settings.Field.CallbackParams<SignupCustomFieldAttrs>) => {
    const fieldType =
      getAttrValue({
        attr: params?.attrs?.fieldItem?.advanced?.type,
        baseBreakpoint: params?.baseBreakpoint,
  ...
  const fieldTypeRadioCallback = (params: Module.Settings.Field.CallbackParams<SignupCustomFieldAttrs>) => {
    const fieldType =
      getAttrValue({
        attr: params?.attrs?.fieldItem?.advanced?.type,
        baseBreakpoint: params?.baseBreakpoint,
  ...
  const fieldTypeSelectCallback = (params: Module.Settings.Field.CallbackParams<SignupCustomFieldAttrs>) => {
    const fieldType =
      getAttrValue({
        attr: params?.attrs?.fieldItem?.advanced?.type,
        baseBreakpoint: params?.baseBreakpoint,
  ...
  const isTitleReadOnly =
    isPredefined && !includes(['getresponse', 'sendinblue', 'constant_contact', 'fluentcrm'], selectedProvider);
  ...
  if (groupConfiguration?.contentMainContent?.component?.props) {
    set(
      ...
      [
        ...
        'props',
        'fields',
        'fieldItemAdvancedPredefinedfield',
        'component',
        'props',
        'options',
      ],
      predefinedFieldsOptions,
    );
    set(
      groupConfiguration,
      [
    ...
    set(
      ...
      onChangePredefinedSelect,
    );
    set(
      groupConfiguration,
      ['contentMainContent', 'component', 'props', 'fields', 'fieldItemAdvancedPredefinedfield', 'render'],
      isPredefined,
    ); // Usably, We use `visible` prop to hide the field, but in this case we need to use `render`.
    set(
      groupConfiguration,
      ['contentMainContent', 'component', 'props', 'fields', 'fieldItemAdvancedId', 'component', 'props', 'readonly'],
      isPredefined,
    );
    set(
      groupConfiguration,
      ['contentMainContent', 'component', 'props', 'fields', 'fieldItemInnercontent', 'component', 'props', 'readonly'],
      isTitleReadOnly,
    );
  }
  ...
  if (groupConfiguration?.contentConditionalLogic?.component?.props) {
    ...
    set(
      groupConfiguration,
      ['contentConditionalLogic', 'component', 'props', 'fields', 'conditionalLogicInnercontent', 'visible'],
      conditionalLogicEnableCallback,
    );
    set(
      groupConfiguration,
      ['contentConditionalLogic', 'component', 'props', 'fields', 'conditionalLogicAdvancedRelation', 'visible'],
      conditionalLogicEnableCallback,
    );
  }
  ...
  if (groupConfiguration?.contentFieldOptions?.component?.props) {
    set(
      groupConfiguration,
      ['contentFieldOptions', 'component', 'props', 'fields', 'fieldItemAdvancedAllowedsymbols', 'visible'],
      fieldTypeInputCallback,
    );
    set(
      groupConfiguration,
      ['contentFieldOptions', 'component', 'props', 'fields', 'fieldItemAdvancedMaxlength', 'visible'],
      fieldTypeInputCallback,
    );
    set(
      groupConfiguration,
      ['contentFieldOptions', 'component', 'props', 'fields', 'fieldItemAdvancedMinlength', 'visible'],
      fieldTypeInputCallback,
    );
    set(
      groupConfiguration,
      ['contentFieldOptions', 'component', 'props', 'fields', 'fieldItemAdvancedCheckboxoptions', 'visible'],
      fieldTypeCheckboxCallback,
    );
    set(
      groupConfiguration,
      [
        ...
        'props',
        'fields',
        'fieldItemAdvancedCheckboxoptions',
        'component',
        'props',
        'firstValue',
      ],
    ...
    set(
      groupConfiguration,
      ['contentFieldOptions', 'component', 'props', 'fields', 'fieldItemAdvancedRadiooptions', 'visible'],
      fieldTypeRadioCallback,
    );
    set(
      groupConfiguration,
      ['contentFieldOptions', 'component', 'props', 'fields', 'fieldItemAdvancedSelectoptions', 'visible'],
      fieldTypeSelectCallback,
    );
  }
