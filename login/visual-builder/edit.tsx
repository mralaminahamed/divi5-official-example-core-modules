...
const LoginEdit = (props: LoginEditProps): ReactElement => {
  ...
  const renderLoginForm = () => {
    const redirectInputMarkup = <input type="hidden" name="redirect_to" value={currentPageUrl} />;

    const redirectInput = redirectCurrentPage ? redirectInputMarkup : '';

    // Show the full login form in VB.
    return (
      <div className="et_pb_newsletter_form et_pb_login_form">
        <form action={loginFormUrl} method="post">
          <input
            type="text"
            defaultValue={__(
              'Note: this field is used to disable browser autofill during the form editing in VB',
              'et_builder_5',
            )}
            name="log"
            style={{ display: 'none' }}
          ...
          <input
            ...
            style={{ display: 'none' }}
          />
          <p className="et_pb_contact_form_field">
            <label className="et_pb_contact_form_label" htmlFor={`user_login_${id}`} style={{ display: 'none' }}>
              {__('Username', 'et_builder_5')}
            </label>
            {elements.render({
              attrName: 'field',
              tagName: 'input',
              ...
              htmlAttributes: {
                id: `user_login_${id}`,
                placeholder: __('Username', 'et_builder_5'),
                type: 'text',
          ...
          <p className="et_pb_contact_form_field">
            <label className="et_pb_contact_form_label" htmlFor={`user_pass_${id}`} style={{ display: 'none' }}>
              {__('Password', 'et_builder_5')}
            </label>
            {elements.render({
              attrName: 'field',
              tagName: 'input',
              ...
              htmlAttributes: {
                id: `user_pass_${id}`,
                placeholder: __('Password', 'et_builder_5'),
                type: 'password',
                onChange: (e: React.ChangeEvent<HTMLInputElement>) => {
          ...
          <p className="et_pb_forgot_password">
            <a href={forgotPasswordUrl}>{__('Forgot your password?', 'et_builder_5')}</a>
          </p>
          <p>
            {elements.render({
              attrName: 'button',
