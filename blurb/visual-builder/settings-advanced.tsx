...
export const SettingsAdvanced = ({ groupConfiguration }: Module.Settings.Panel.Props<BlurbAttrs>): ReactElement => {
  const showAttributesCallback = (
    ...
    // Show Attributes.
    const showAttributes = params?.isActivePreset
      ? true
      : 'on' !== params?.attrs?.imageIcon?.innerContent?.desktop?.value?.useIcon;
