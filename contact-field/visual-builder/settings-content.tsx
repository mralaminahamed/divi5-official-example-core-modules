...
import { select, useDispatch, useSelect } from '@divi/data';
...
export const SettingsContent = ({
  ...
  const { fieldItemAttr } = useSelect(selectStore => ({
    ...
    if (currentFieldId && /\s/.test(currentFieldId)) {
      ...
      if (fieldIdAttr && fieldIdAttr.setIn && 'function' === typeof fieldIdAttr.setIn) {
        editModuleAttribute({
          ...
          subName: false, // Skip detection - fieldItem.advanced.id is a simple string value, no subName concept
        });
      }
    }
  };
  ...
  const conditionalLogicEnableCallback = (params: Module.Settings.Field.CallbackParams<ContactFieldAttrs>) => {
  ...
  const fieldTypeInputCallback = (params: Module.Settings.Field.CallbackParams<ContactFieldAttrs>) => {
    const fieldType =
      getAttrValue({
  ...
  const fieldTypeCheckboxCallback = (params: Module.Settings.Field.CallbackParams<ContactFieldAttrs>) => {
    const fieldType =
      getAttrValue({
        attr: params?.attrs?.fieldItem?.advanced?.type,
        baseBreakpoint: params?.baseBreakpoint,
  ...
  const fieldTypeRadioCallback = (params: Module.Settings.Field.CallbackParams<ContactFieldAttrs>) => {
    const fieldType =
      getAttrValue({
        attr: params?.attrs?.fieldItem?.advanced?.type,
        baseBreakpoint: params?.baseBreakpoint,
  ...
  const fieldTypeSelectCallback = (params: Module.Settings.Field.CallbackParams<ContactFieldAttrs>) => {
    const fieldType =
      getAttrValue({
        attr: params?.attrs?.fieldItem?.advanced?.type,
        baseBreakpoint: params?.baseBreakpoint,
  ...
  if (groupConfiguration?.contentText?.component?.props) {
    set(
      groupConfiguration,
      ['contentText', 'component', 'props', 'fields', 'fieldItemAdvancedId', 'component', 'props', 'onBlur'],
      onBlurFieldIdCallback,
    );
  }
