...
import {
  buttonAlignmentAnchorDeclaration,
  buttonAlignmentWrapperDeclaration,
  buttonIconStyleDeclaration,
  buttonSpacingDeclaration,
  positionWidthStyleDeclaration,
} from './style-declarations';
import { removeIconAttrValue } from './utils';
...
const ModuleStyles = <TProps extends StylesProps<ButtonAttrs>>({
  ...
  // Note: attrs are already resolved by edit-container.tsx and register-module/index.ts
  // before being passed to ModuleElements, so $variable(...)$ tokens are already converted.
  ...
  // TODO: feat(D5, refactor): Probably no longer necessary.
  const iconSelectorFunction = <TParams extends SelectorFunctionParams<TParams['attr']>>(params: TParams): string =>
    getIconHoverSelector(buttonIconSelectorFunction(params));
  const iconStyleSelectorFunction = <TParams extends SelectorFunctionParams<TParams['attr']>>(
    params: TParams,
  ): string => getIconHoverSelector(params?.selector);

  const buttonAffectingAttrs =
    'module' === styleGroup
      ? {
          spacing: merge(
            {},
            elements?.presetPrintedStyleAttrs?.module?.decoration?.spacing,
  ...
  // Render style content for the current state.
  const renderStyleContent = () => (
    <React.Fragment>
          ...
          advancedStyles: [
            ...
            },
            {
              ...
              props: {
                ...
                declarationFunction: buttonAlignmentWrapperDeclaration,
              },
            },
            {
              componentName: 'divi/common',
              props: {
                selector: `${wrapperOrderClass} ${baseOrderClass}`,
                attr: attrs?.module?.advanced?.alignment,
                declarationFunction: buttonAlignmentAnchorDeclaration,
              },
            },
            {
              componentName: 'divi/common',
              props: {
                ...
                declarationFunction: positionWidthStyleDeclaration,
              },
            },
          ],
        },
      ...
      {elements.style({
        ...
        styleProps: {
          ...
          button: {
            ...
            affectingAttrs: buttonAffectingAttrs,
          },
          spacing: {
            important: true,
          },
          attrsFilter: decorationAttr => {
            ...
            if ('presetGroup' === styleGroup && decorationAttr?.button) {
              return {
                ...decorationAttr,
                button: removeIconAttrValue(decorationAttr.button),
              };
            }
       ...
       */}
      <CssStyle selector={orderClass} attr={attrs.css} />
    </React.Fragment>
  );
