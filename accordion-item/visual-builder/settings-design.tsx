...
export const SettingsDesign = ({
  ...
  const { parentTitleFontAttr, parentClosedToggleTitleFontAttr } = useSelect(selectStore => {
    const parentModuleId = selectStore('divi/edit-post').getParentModuleId(moduleId);

    return {
      parentTitleFontAttr: selectStore('divi/edit-post').getModuleAttr<AccordionAttrs['title']['decoration']['font']>(
        parentModuleId,
        'title.decoration.font',
      ),
      parentClosedToggleTitleFontAttr: selectStore('divi/edit-post').getModuleAttr<
        AccordionAttrs['closedToggle']['decoration']['font']
      >(parentModuleId, 'closedToggle.decoration.font'),
    };
  });

  let defaultTitleFont = defaultSettingsAttrs?.title?.decoration?.font?.asMutable({ deep: true }) ?? {};
  let defaultClosedToggleTitleFont =
    defaultSettingsAttrs?.closedToggle?.decoration?.font?.asMutable({
      deep: true,
    }) ?? {};
  ...
  if (groupConfiguration?.designClosedToggleText?.component?.props) {
    set(
      groupConfiguration,
      ['designClosedToggleText', 'component', 'props', 'fields', 'font', 'component', 'props', 'defaultGroupAttr'],
      defaultClosedToggleTitleFont,
    );
  }
